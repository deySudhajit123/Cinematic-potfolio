...
const ISSUER_ICON_MAP = {
  'Google':    <SiGoogle     size={10} />,
  'Microsoft': <FaMicrosoft  size={10} />,
  'IBM':       <span className={styles.issuerIbmLabel}>ibm</span>,
}

function getIssuerIcon(issuer) {
  const primary = issuer.split('·')[0].trim()
  return ISSUER_ICON_MAP[primary] ?? null
}
  ...
  useEffect(() => {
    ...
    function onScroll() {
      ...
      if (isMobile) {
        ...
        // ── Phase 3: keep image visible, no crossfade ──
    ...
    <div ref={wrapperRef} className={styles.wrapper}>
      <div ref={stickyRef} className={styles.sticky}>
        ...
        <div className={styles.mobileFooterBg}>
          <Image
            src="/assets/my_standing_pic.jpg"
            alt=""
            fill
            quality={100}
        ...
        {/* ── Certifications content (right of image) ── */}
        <div ref={pubContentRef} className={styles.pubContent}>
          ...
          <div className={styles.pubHero}>
            <p  ref={labelRef}   className={styles.label}>Credentials &amp; Badges</p>
          ...
          <div className={styles.list}>
            {CERTS.map((cert, i) => (
              <a
                ...
                className={`${styles.item} ${cert.inProgress ? styles.itemActive : ''} ${cert.isBadge ? styles.itemBadge : ''}`}
              >
                  ...
                  <div className={styles.itemTop}>
                    ...
                    <span className={styles.platform}>
                      {getIssuerIcon(cert.issuer) && (
                        <span className={styles.issuerIcon}>{getIssuerIcon(cert.issuer)}</span>
                      )}
                      {cert.issuer}
                    </span>
                    {cert.isBadge && (
                      <span className={styles.badgePill}>🏅 Digital Badge</span>
                    )}
                    {cert.inProgress && (
                ...
                <div className={styles.itemRight}>
                  ...
                  {!cert.inProgress && (
                    <span className={styles.readBtn}>
                      {cert.isBadge ? 'View Badge' : 'Verify'} <FiArrowUpRight size={11} />
                    </span>
                  )}
                </div>
              </a>
